...
describe('modules/common/service.js', function () {
  ...
  it('listCity 应该正常返回城市列表:', function (done) {
    service.listCity().then(function (data) {
      assert(_.isArray(data.list), '服务器返回列表数据异常');
      done();
    });
  });
  ...
  it('listProject 应该正常返回社区列表:', function (done) {
    service.listCity().then(function (data) {
      assert(data.list.length > 0, '城市列表不能为空');

      var city = null;
      _.forOwn(data.list[0], function (value) {
        city = value[0];
      });
      return city.id;
    }).then(function (cityId) {
      service.listProject(cityId).then(function (data) {
        assert(_.isArray(data.list), '服务器返回列表数据异常');
        done();
      });
    });
  ...
  it('listBuilding 应该正常返回社区列表:', function (done) {
    service.listCity().then(function (data) {
      ...
      assert(data.list.length > 0, '项目列表不能为空');
  ...
  it('listRoom 应该正常返回房间列表:', function (done) {
    ...
    service.listCity().then(function (data) {
      ...
      assert(data.list.length > 0, '楼栋列表不能为空');
      var building = data.list[0];
      service.listRoom(cityId, building.projectId, building.id).then(function (data) {
        assert(_.isArray(data.list), '服务器返回列表数据异常');
        done();
      });
    });
